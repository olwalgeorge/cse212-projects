...
public class LinkedList : IEnumerable<int>
{
    ...
    public void Remove(int value)
    {
        ...
        while (curr is not null)
        {
            if (curr.Data == value)
            {
                // If this is the only node in the list, remove it by setting head and tail to null
                if (curr == _head && curr == _tail)
                {
                    _head = null;
                    _tail = null;
                }
                // If this is the head node, use RemoveHead logic
                else if (curr == _head)
                {
                    RemoveHead();
                }
                // If this is the tail node, use RemoveTail logic
                else if (curr == _tail)
                {
                    RemoveTail();
                }
                ...
                return; // We can exit the function after we remove the first occurrence
            }
    ...
    {
        // TODO Problem 4
        // Search through all nodes starting at the head of the list.
        Node? curr = _head;
        while (curr is not null)
        {
            if (curr.Data == oldValue)
            {
                ...
                // Continue searching for more instances (don't return)
            }

            curr = curr.Next; // Go to the next node to search for more 'oldValue'
        }
    }
